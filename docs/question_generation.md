...
## 7.5 생성 규모·비용 기준 (EP당 ~20)

**기준: 문법·활용 드릴은 EP당(항목/폼별) ~20개.** 규칙은 하나인데 단어만 바뀌는 반복이라 20개면 충분하고, **Codex(luna) 비용** 때문에 과생성은 부담. 어휘는 이미 완성(창고 유지, 등급 필터로 초급만 서빙 — 학습자에겐 34k가 아니라 초급 풀만 보임).

| 항목 | 엔진 | 규모 | 비용 |
|---|---|---|---|
| 어휘 | 로직(완성) | 창고 전체, 서빙=등급필터 | 0 |
| 조사(T1) | 로직 | **EP·조사별 ~20** | 0 |
| 규칙활용(T2) | 로직엔진 | **EP당 ~20** | 0 |
| 불규칙활용 | luna 보조 | **EP당 ≤20** | 💰 |
| 예문·뉘앙스(T3) | luna 저작 | **EP당 ≤20** | 💰 |

**샘플링(규칙 커버 우선)**: 20개를 랜덤이 아니라 **규칙 케이스를 덮게** 층화 선택 — 조사=받침있음/없음((으)로는 ㄹ예외 포함), 활용=모음조화·축약·ㅡ탈락 다양하게. 적은 수로 규칙을 다 보여주는 게 목표.

**luna 비용 통제 (필수)**:
- **EP당 1 배치콜** (`--output-schema`로 N개 한 번에). 문제마다 호출 금지.
- **캐시·멱등**: luna 산출 JSON 저장 → 재생성은 캐시 사용. 입력(단어·프롬프트) 안 바뀌면 재호출 X.
- **검증 게이트**: 로직 재계산(받침·정답)·스키마·중복 통과분만 채택, 실패는 flag.
